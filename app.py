...
import pandas as pd
import io
import plotly.express as px  # ต้องติดตั้ง: pip install plotly
import re
...
with st.sidebar:
    ...
    st.info("**AI Assistant**\n\n(Coming Soon: ระบบ AI ช่วยวิเคราะห์ความผิดปกติ)")
    ...
    with st.expander("วิธีการใช้งาน"):
        ...
        2. **โหมดไฟล์เดียว:** แสดงกราฟเชิงลึก, Heatmap และค้นหา Repeats
        3. **โหมดเปรียบเทียบ:** อัปโหลดหลายไฟล์เพื่อดูตาราง Ranking และ Interactive Graph
        """)
...
if not uploaded_files:
    ...
    if len(results) == 1:
        ...
        with c1:
            st.markdown("**1️⃣ การกระจายตัวของ Intergenic Length**")
        ...
        with c2:
            st.markdown("**2️⃣ GC-Content Comparison**")
        ...
        # 3. Sliding Window
        st.markdown("**3️⃣ GC% Variation (Sliding Window)**")
        window = 1000
        seq = data['seq']
        pos = []
        vals = []
    ...
    # ============================================
        ...
        with c1:
            ...
            ax_bar.barh(df_sorted["Organism"], df_sorted["Non-coding %"], color="#f472b6") # Pink
        ...
        with c2:
            ...
            ax_bar2.barh(df_sorted_len["Organism"], df_sorted_len["Length (bp)"], color="#60a5fa") # Blue
